...
if (isset($_GET['messageID']) && filter_var($_GET['messageID'], FILTER_VALIDATE_INT)) {
    ...
    $deleteQuery = "DELETE FROM JavaChat WHERE MessageID = :messageID AND UserID = :loggedInUserID";
    ...
    header('Location: java_chat.php');  // Redirects back to the chatroom after message is deleted
    exit();
} else {
    header('Location: java_chat.php'); // Redirects back to the chatroom if messageID is not set or invalid
    exit();
}
?>
