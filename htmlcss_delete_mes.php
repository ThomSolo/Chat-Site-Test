...
if (isset($_GET['messageID']) && filter_var($_GET['messageID'], FILTER_VALIDATE_INT)) {
    ...
    $deleteQuery = "DELETE FROM HTMLCSSChat WHERE MessageID = :messageID AND UserID = :loggedInUserID";
    ...
    header('Location: htmlcss_chat.php'); // Redirects back to the chatroom after message is deleted
    exit();
} else {
    header('Location: htmlcss_chat.php'); // Redirects back to the chatroom if messageID is not set or invalid
    exit();
}
?>
